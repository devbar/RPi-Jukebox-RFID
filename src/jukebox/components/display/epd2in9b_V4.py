...
class Epd2in9bV4Display:
    ...
    def show(self, title: str, artist: str, album: str = None, paused: bool = False, repeat_info: str = None):
        ...
        HImage = Image.new('1', (epd.height, epd.width), 255)  # 298*126
        ...
        if repeat_info == 'repeat_one':
            ...
            draw.text((240, 99), "[RA]", font = font18, fill = 0)
        else:
            draw.text((240, 99), "   ", font = font18, fill = 0)
            
        if paused:
            draw.text((270, 99), "[P]", font = font18, fill = 0)
        elif repeat_info == 'repeat_all':
            draw.text((270, 99), "   ", font = font18, fill = 0)
            
        HImageRed = Image.new('1', (epd.height, epd.width), 255)  # 298*126        

        #epd.display_Fast(epd.getbuffer(HImage),epd.getbuffer(HImageRed))        
        epd.display_Partial(epd.getbuffer(HImage), 5, 5, epd.width - 5, epd.height - 5)
        #epd.sleep()
